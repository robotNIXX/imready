    ...
    <md-dialog :md-active.sync="showSignup" class="modal-signup">
        ...
        <md-dialog-content>
            <form novalidate class="md-layout" @submit.prevent="register" method="post">
                <div class="md-layout">
                    <md-field :class="messageClass">
                        <md-input v-model="userRegister.name" name="name" placeholder="Имя пользователя"></md-input>
                        <span class="md-error" v-if="has_error && errors.name" v-for="error in errors.name">{{ error }}</span>
                    </md-field>
                    <md-field :class="messageClass">
                        <md-input v-model="userRegister.email" name="email" type="email" placeholder="Адрес электронной почты"></md-input>
                        <span class="md-error" v-if="has_error && errors.email" v-for="error in errors.email">{{ error }}</span>
                    </md-field>
                    <md-field :class="messageClass">
                        <md-input v-model="userRegister.password" name="password" type="password" placeholder="Пароль" required></md-input>
                        <span class="md-error" v-if="has_error && errors.password" v-for="error in errors.password">{{ error }}</span>
                    </md-field>
                    <md-field :class="messageClass">
                        <md-input v-model="userRegister.password_confirmation" name="password" type="password" placeholder="Подтвердите пароль" required></md-input>
